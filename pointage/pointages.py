...
def set_time(summary, time, legal_time):
	''' set time_enter time_out for a given summary '''
	if legal_time.full_time:
		if summary.in_morning is None:
			summary.in_morning = time
		else:
			summary.out_morning = time
		summary.save()
	else:
		if summary.in_morning is None:
			summary.in_morning = time
		elif summary.out_morning is None:
			summary.out_morning = time
		elif summary.in_evening is None:
			summary.in_evening = time
		else:
			summary.out_evening = time

def set_status(summary, legal_time):
	if summary.status1 != 'break':
		if summary.in_morning is None or summary.out_morning is None:
		...
		if not legal_time.full_time:
			if summary.in_evening is None or summary.out_evening is None:
...
def set_hours(summary, legal_time):
	...
	if summary.in_evening is not None and summary.out_evening is not None:
		if summary.status2 != 'break':
			if summary.status2 == 'late':
				nb_hours += legal_time.time_out_evening.hour - summary.in_evening.hour
...
def show_pointages(start, end, salary, Summary):
