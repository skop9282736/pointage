...
from .pointages import exists, set_hours, set_status, set_time
...
class AtdRecord_clone(models.Model):
    ...
    def __str__(self):
        return "{} , {} ==> {}".format(
            self.RecDate, self.RecTime, self.CardNo.first_name
        )


class AtdRecord(models.Model):
    # this class to clone the atdrecord original data cuz the c# app send data directly to db
    # and it is not going thro django triggers aka signals
    SerialId = models.IntegerField(unique=True)  # unique
    CardNo = models.ForeignKey(Salary, on_delete=models.SET_NULL, null=True)  # salary
    RecDate = models.DateField()
    RecTime = models.TimeField(max_length=255)
    dateb = models.DateTimeField(max_length=255)
